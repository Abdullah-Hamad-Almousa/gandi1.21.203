...
public class StormWand extends Item {
    ...
    private static final Map<Block, Block> STORM_MAP =
            Map.of(
                    ModBlocks.CORRUPTED_BLOCK_A.get(), ModBlocks.CORRUPTED_BLOCK_B.get(),
                    ModBlocks.CORRUPTED_BLOCK_B.get(), ModBlocks.CORRUPTED_BLOCK_A.get()
            );

    public StormWand(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (STORM_MAP.containsKey(clickedBlock)){

            if (!level.isClientSide()){

                level.setBlockAndUpdate(context.getClickedPos(), STORM_MAP.get(clickedBlock)
                        .defaultBlockState());
